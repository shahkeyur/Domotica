...
void DomoticaClass::_onWifiDisconnect(const WiFiEventStationModeDisconnected& event) {
    Interface::get().getLogger() << F("Disconnected from Wi-Fi.") << endl;
  _wifiReconnectTimer = _reconnectTimer.once(3000, std::bind(&DomoticaClass::_connectWifi, this));
  Interface::get().getLogger() << F("WiFi Reconnect Count:")<< _wifiReconnectCounter <<endl;
  if (_wifiReconnectCounter++ > WIFI_RECONNECT_MAX_BACKOFF) {
    Interface::get().getLogger() << F("Cannot connect to wifi...") << endl << F("Rebooting...") << endl;
    ESP.restart();
  };
}
...
void DomoticaClass::_connectMqtt()
{
  ...
  if ((String(_config.get().mqtt.server.host)).endsWith(".local")) {
    ...
    _mqttClient.setServer(_config.get().mqtt.server.host, _config.get().mqtt.server.port);
  }

  if (_config.get().mqtt.auth) {
    _mqttClient.setCredentials(_config.get().mqtt.username,_config.get().mqtt.password);
  }
  Interface::get().getLogger() << F("Connecting to MQTT Broker(") << _config.get().mqtt.server.host << F(":") << _config.get().mqtt.server.port << F(")") << endl;
...
void DomoticaClass::_onMqttDisconnect(AsyncMqttClientDisconnectReason reason) {
  ...
  if (WiFi.isConnected()) {
    _mqttReconnectTimer = _reconnectTimer.once(3000, std::bind(&DomoticaClass::_connectMqtt,this));
  }
  Interface::get().getLogger() << F("MQTT Reconnect Count:")<< _mqttReconnectCounter <<endl;
...
void DomoticaClass::_setupCallbacks(){
  _wifiConnectHandler = WiFi.onStationModeGotIP(std::bind(&DomoticaClass::_onWifiConnect, this,std::placeholders::_1));
  _wifiDisconnectHandler = WiFi.onStationModeDisconnected(std::bind(&DomoticaClass::_onWifiDisconnect, this,std::placeholders::_1));
  Interface::get().getMqttClient().onConnect(std::bind(&DomoticaClass::_onMqttConnect, this,std::placeholders::_1));
...
bool DomoticaClass::isConnected() {
...
AsyncMqttClient& DomoticaClass::getMqttClient(){
   return _mqttClient;
}

Logger& DomoticaClass::getLogger(){
  return _logger;
}
